...
Dim theDateModified     As String = "30.07.2008"
...
'	RegisterParameterString("theAnimStopFlag", "animation stop flags [1|1#...]:", "1|1|1|0|1|1|1|1", 55, 55, "")
	...
	RegisterInfoText(strInfoText)
	RegisterPushButton("btSetMirror", "Spiegelung an / aus", 41)
	RegisterInfoText(strInfoText)
...
'Sub _updateScene_assignValueDiffLabel_3(contLabelBase As Container, strTypeOfGraphics As String, strValueP As String, strValueD As String, strLabel1 As String, strLabel2 As String, strLabel3 As String)
			...
			Scene.dbgOutput(1, strDebugLocation, "[tmpMaterial]: [MATERIAL*ZDFWahlen_v3/9_SHARED/material/" & sGraphicsData.aGroup[iGroup].aMaterial[iElement] & "]")
			...
			tmpMaterial = contElement.FindSubContainer("$DATA$obj_geom_mirror").CreateMaterial("MATERIAL*ZDFWahlen_v3/9_SHARED/material/" & sGraphicsData.aGroup[iGroup].aMaterial[iElement] ) 
...
Sub	setMirror()
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		For iElement = 0 To sGraphicsData.aGroup[iGroup].nElements - 1
			...
			If contElement.FindSubContainer( kDataSubPath & kBarColoredSubPathMirror ).Active = False then
				contElement.FindSubContainer( kDataSubPath & kBarColoredSubPathMirror ).Active = True
			Else
				contElement.FindSubContainer( kDataSubPath & kBarColoredSubPathMirror ).Active = False		
			End If
		Next
	Next	
