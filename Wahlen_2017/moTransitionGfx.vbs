...
Dim blnRunBGAnimation As Boolean = FALSE

' background transition
Dim nextBGImage, prevBGImage As String
...
'	RegisterParameterString("theTypeOfGraphics", "type of graphics:", "/CLEAR", 50, 100, "")
	...
	RegisterParameterString("theBackgroundImg", "background image:", "back_bundesadler", 50, 100, "")
...
Sub updateScene_TransitionAnimation()
	...
	Dim contEnvironment As Container
	Dim strAniTrio As String
	Dim dblKeyTime As Double
	Dim dirAniIO, dirAniData, dirAniBackground As Director
...
'println "DEBUG: prev=gPBRK - [" & "#" & dirAniData.vizID & " SHOW $END]" 
	...
	' prepare headline animation
'	contHeadlineScript.ScriptPluginInstance.PushButton("btAssignValues")
	System.SendCommand("#" & dirAnimHeadline.vizID & " SHOW $OUT")

	' prepare background transition
	contEnvironment = Scene.FindContainer("ELE_ENVIRONMENT_2017")
	nextBGImage = GetParameterString("theBackgroundImg")
	If nextBGImage.Match( prevBGImage ) Then
		' no background transition
		contEnvironment.FindSubContainer( "imgBackgroundOUT" ).CreateTexture( "IMAGE*ZDFWahlen_2017/9_SHARED/image/backgrounds/" & prevBGImage )
		blnRunBGAnimation = FALSE
	Else
		' do background transition
		contEnvironment.FindSubContainer( "imgBackgroundOUT" ).CreateTexture( "IMAGE*ZDFWahlen_2017/9_SHARED/image/backgrounds/" & prevBGImage )
		contEnvironment.FindSubContainer( "imgBackgroundIN" ).CreateTexture( "IMAGE*ZDFWahlen_2017/9_SHARED/image/backgrounds/" & nextBGImage )
		blnRunBGAnimation = TRUE
	End If
	prevBGImage = nextBGImage
	' prepare background transition
	dirAniBackground = Stage.FindDirector("ANI_BG_TRANSITION")
	System.SendCommand("#" & dirAniBackground.vizID & " SHOW $OUT")
...
Sub updateScene_RunTransitionAnimation()
	...
	' run background transition
	If blnRunBGAnimation = TRUE Then
		dirAniBackground = Stage.FindDirector("ANI_BG_TRANSITION")
		System.SendCommand("#" & dirAniBackground.vizID & " GOTO_TRIO $OUT $IN")
	End If
...
' PLAYOUT KOAL SPLIT ANIMATION
' scene._POA_PlayGoToTrioALL( "KOAL_SPLIT", "EventIN")
'
' scene._POA_PlayGoToTrioIdx( "KOAL_SPLIT", "EventIN", maxAnimOrderFlag )
		Dim iMax As Integer
		iMax = getMaxAnimationIndex()
		If idxAnim = iMax+1 Then
			scene._POA_PlayGoToTrioIdx( "KOAL_SPLIT", "EventIN", iMax+1  )
		End If
...
Sub updateScene_runBarAnimShowStart()
	...
	Dim dirAniData, dirAniHelper As Director
...
'			Scene.dbgOutput(1, strDebugLocation, "[iAnim] [strDirectorName] [strDirectorVizID]: ["	& iAnim & "] [" & Scene.aPlayoutAnimationIN[iAnim].strDirectorName & "] [" & Scene.aPlayoutAnimationIN[iAnim].dirAnimation.vizID & "]")
	...
	If strHelp.Match("gHRSVS|gHRSVK") Then
		scene._POA_PlayShowF0ALL( "KOAL_SPLIT", "EventIN" )
		contBlenderIN   = Scene.FindContainer("$ALL$CONTENT$ELE_PLAYOUT$ELE_ANIMATION$moBLENDER$IN$ELEMENT")
		dirAniHelper = contBlenderIN.GetDirectorOfMergedGeometry()
		System.SendCommand("#" & dirAniHelper.FindSubDirector("$LABEL_SV").vizID & " SHOW $IN")
'		System.SendCommand("#" & dirAniHelper.FindSubDirector("$LABEL_SV$ANI_IO").vizID & " SHOW $IN")
