...
Dim theDateModified     As String = "19.04.2017"
...
'	04.10.2010 (ie) Button Prozente an / aus zugefuegt
'	23.06.2014 (ie) Labelpositionierung zugefuegt Label 2 / 3 -> Label 1 noch nicht verwendet
...
Sub readGraphicsData()
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		sGraphicsData.dblMaxValue = Scene._getMaxStrArrayValue( sGraphData.aValueNum, sGraphicsData.dblMaxValue )
		sGraphicsData.dblMinValue = Scene._getMinStrArrayValue( sGraphData.aValueNum, sGraphicsData.dblMinValue )

		dblTemp = Scene._getMaxBaxValue( sGraphData.aValuePos ) - 1
		If dblTemp > sGraphicsData.dblMaxPos Then
			sGraphicsData.dblMaxPos  = dblTemp
		End If
...
Sub updateScene_assignData()
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		For iElement = 0 To sGraphicsData.aGroup[iGroup].nElements - 1 
			' calc x value
			dblValueX = (CDbl( sGraphicsData.aGroup[iGroup].aValuePos[iElement] ) - 1) * kMaxVizWidthUMKV / sGraphicsData.dblMaxPos
			strValueX = strValueX & DoubleToString(dblValueX, 4) & "|"

			' calc y value
			dblValueY = (CDbl( sGraphicsData.aGroup[iGroup].aValueNum[iElement] ) - sGraphicsData.dblMinValue) * sGlobalParameter.dblMaxVizValueUMKV / ( sGraphicsData.dblMaxValue - sGraphicsData.dblMinValue )
			strValueY = strValueY & DoubleToString(dblValueY, 4) & "|"

'			' update graph label - text but not position
			If iElement = 0 Then
...
Sub	setGraphLabelPosXY()
	...
	Dim contGroup, contLabel As Container
	Dim tmpGroupName, tmpString As String
	Dim dblValueX, dblValueY, dblYcoo, dblXcoo As Double
	Dim iGroup, iElement, iLabel, nLabels As Integer
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		For iElement = 0 To sGraphicsData.aGroup[iGroup].nElements - 1
			...
			dblXcoo = 43 + 1.177 * dblValueX
			dblYcoo = 27.594 + 1.179 * dblValueY
			println "GraphPoint x=" & dblValueX & " y=" & dblValueY & "-->" & dblXcoo & ";" & dblYcoo

			' update graph label
			If iElement = 0 Then
				contGroup.FindSubContainer("$LABEL_L").Position.Y = dblYcoo
			ElseIf iElement = sGraphicsData.aGroup[iGroup].nElements-1 Then
				contGroup.FindSubContainer("$LABEL_R").Position.Y = dblYcoo
			ElseIf sGraphicsData.aGroup[iGroup].aValueTxt[iElement].EndsWith("*") Then
				contLabel = contGroup.FindSubContainer("$LABEL_M$LABEL_" & CStr(nLabels) )
				contLabel.Active = TRUE
				' set position
				contLabel.Position.X = dblXcoo
				contLabel.Position.Y = dblYcoo
...
Sub	setLabelPosIngo(whichGroup As Integer, whichLabel As String, offset As Integer)
	...
	if whichLabel = "L" then
		'Y-Pos fuer ersten Wert berechnen
		dblValueY = CDbl( sGraphicsData.aGroup[whichGroup].aValueNum[0] )
	else 
		'Y-Pos fuer letzten Wert berechnen
	...
	dblValueY = (dblValueY - sGraphicsData.dblMinValue) * sGlobalParameter.dblMaxVizValueUMKV / ( sGraphicsData.dblMaxValue - sGraphicsData.dblMinValue )
...
Sub	setGraphLabelPerc()
	...
	'UUID der Geometrie (enthaelt Name und eindeutige Zuordung im DB-Pfad
	geom_uuid = tmp.Geometry.Uuid.FullString
	'Splitten und Mergen - braucht man damit eine Veraenderung erkannt wird und der Speicherprozess moeglich ist
