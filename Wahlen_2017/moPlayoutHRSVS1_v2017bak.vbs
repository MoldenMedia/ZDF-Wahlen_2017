...
'-Dim kMaxLabelWidth           As Double = 85.0
...
'-Dim kPieLabelLines4_PosY As Double = 0
'-Dim kPieLabelLines3_PosY As Double = 2
'-Dim kPieLabelLines2_PosY As Double = 4
'-Dim kPieLabelLines1_PosY As Double = 6
'-Dim kPieLabelLines0_PosY As Double = 6
...
Sub readGraphicsData()
	...
	Dim iPie, iGroup, iElement, nMaxLabel As Integer
...
'-		fGlobPieLabelLinesPosY = kPieLabelLines0_PosY
		If aEleLabel1[iGroup] <> "" Then
			nMaxLabel = 1
'-			fGlobPieLabelLinesPosY = kPieLabelLines1_PosY
'			sGroupData.dblLabelPosY = kPieLabelLines1_PosY
		End If
		If aEleLabel2[iGroup] <> "" Then
			nMaxLabel = 2
'-			fGlobPieLabelLinesPosY = kPieLabelLines2_PosY
		End If
		If aEleLabel3[iGroup] <> "" Then
			nMaxLabel = 3
'-			fGlobPieLabelLinesPosY = kPieLabelLines3_PosY
		End If
		If aEleLabel4[iGroup] <> "" Then
			nMaxLabel = 4
'-			fGlobPieLabelLinesPosY = kPieLabelLines4_PosY
		End If
...
Sub updateScene_assignData()
	...
	Dim sGraphicsData As structGraphicsData
'-	Dim dblSmallLabelWidthAddition As Double
...
'-Dim kBigLabelWidth   As Double
'-Dim kSmallLabelWidth As Double
'-	kBigLabelWidth = 13.5
...
'-		dblSmallLabelWidthAddition = 4.0
'-		kSmallLabelWidth = 26.0
	Else
...
'-		dblSmallLabelWidthAddition = 1.0
'-		kSmallLabelWidth = 20.0
	End If

	' calc distance between labels
'-	If sGraphicsData.nSmallLabel = 0 Then
'		dblLabelDistance = kMaxLabelWidth / ( sGraphicsData.nBigLabel - 1 )
'-		dblLabelDistance = kMaxLabelWidth / ( sGraphicsData.nBigLabel - 1 )
'-	Else
		' gaps between labels not equal
'		dblLabelDistance = kMaxLabelWidth / ( sGraphicsData.nBigLabel + 0.5 )
'-		dblLabelDistance = ( kMaxLabelWidth + kBigLabelWidth - sGraphicsData.nBigLabel*kBigLabelWidth - kSmallLabelWidth ) / ( sGraphicsData.nBigLabel - 1 ) + kBigLabelWidth
'-		dblLabelDistance = ( kMaxLabelWidth + kBigLabelWidth - sGraphicsData.nBigLabel*kBigLabelWidth - kSmallLabelWidth ) / ( sGraphicsData.nBigLabel ) + kBigLabelWidth
'-println "DEBUG: [kMaxLabelWidth] [kBigLabelWidth] [kSmallLabelWidth] [sGraphicsData.nBigLabel] [dblLabelDistance]: ["	& kMaxLabelWidth & "] [" & kBigLabelWidth & "] [" & kSmallLabelWidth & "] [" & sGraphicsData.nBigLabel & "] [" & dblLabelDistance & "]"
'		dblLabelDistance = kMaxLabelWidth / ( sGraphicsData.nBigLabel )
'-	End If
...
'-				If sGraphicsData.aGroup[iGroup].aTypeOfLabel[iElement] = "S" Then
'-					dblLabelPosX = cntLeft * dblLabelDistance + (kSmallLabelWidth - kBigLabelWidth) / 2.0
'-				Else
...
'-				End If
			...
			ElseIf sGraphicsData.aGroup[iGroup].aAnimDirectionFlag[iElement] = "R" Then
				...
				' calc label positionX
'-				If sGraphicsData.aGroup[iGroup].aTypeOfLabel[iElement] = "S" Then
'-					dblLabelPosX = kMaxLabelWidth - (cntRight * dblLabelDistance) - ( kSmallLabelWidth - kBigLabelWidth ) / 2.0
'-				Else
...
'-				contWork.Position.Y = fGlobPieLabelLinesPosY
'				contWork.Position.Y = sGraphicsData.aGroup[iGroup].dblLabelPosY
...
'				contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVSubPath & strLabelPDSubPath & kLabelSmallSubPath & kLabelSmallLineBase & iElement+1 )
'-				contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVSubPath & strLabelPDSubPath & kLabelSmallSubPath & kLabelSmallLineBase & cntSLbl )
'-				contWork.Active = TRUE
				
				' 090818: bug fix overlapping labels
'-				If cntSLbl = 1 Then
'-					dblLabelSmallPosX = dblLabelPosX 
'-				Else
					If sGraphicsData.aGroup[iGroup].aAnimDirectionFlag[iElement] = "L"	Then
						cntLeft--
					ElseIf sGraphicsData.aGroup[iGroup].aAnimDirectionFlag[iElement] = "R"	Then
...
'-				contWork.Position.X = dblLabelSmallPosX
'				contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVSubPath & strLabelPDSubPath & kLabelSmallSubPath).Position.Y = fGlobPieLabelLinesPosY + 5.0
'-				contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVSubPath & strLabelPDSubPath & kLabelSmallSubPath).Position.Y = fGlobPieLabelLinesPosY + 2.5
'				contWork.Position.Y = sGraphicsData.aGroup[iGroup].dblLabelPosY
'println "DEBUG: [sGraphicsData.strTypeOfGraphic]: [" & sGraphicsData.strTypeOfGraphic & "]..."
				If sGraphicsData.strTypeOfGraphic = "HRSVS1PD" Then
					Scene._updateScene_assignPieLabelPD( contWork, sGraphicsData.aGroup[iGroup].aValueTxt[iElement], sGraphicsData.aGroup[iGroup].aDiffTxt[iElement], aEleValueLabel )
...
End If
			...
			' set topview label 
'-			If iElement < 2 Then
'-				contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVTopSubPath & strLabelPDSubPath & kLabelBigSubPath & kLabelBigLineBase & iElement+1 )
'-				contWork.Active = TRUE
'-			Else
'-				contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVTopSubPath & strLabelPDSubPath & kLabelSmallSubPath & kLabelSmallLineBase & iElement-1 )
'-				contWork.Active = TRUE
'-			End If
			Scene._updateScene_assignPieLabel( contWork, sGraphicsData.aGroup[iGroup].aValueTxt[iElement], aEleValueLabel )
...
If iPie = 0 Then
	...
	Next
	' hide not required small labels
'-	For iElement = sGraphicsData.nSmallLabel To kMaxElementInGroup
'-		contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVSubPath & strLabelPDSubPath & kLabelSmallSubPath & kLabelSmallLineBase & iElement+1 )
'-		contWork.Active = FALSE
'-	Next
End If

	' hide not required small labels in top view
'println "[sGraphicsData.aGroup[iGroup].nElements]: [" & sGraphicsData.aGroup[iGroup].nElements & "] ++++++++++++++++++"	
'-	For iElement = sGraphicsData.nBigLabel + sGraphicsData.nSmallLabel - 2 To kMaxElementInGroup
'-		contWork = contPie.FindSubcontainer( kLabelBaseSubPath & kLabelSVTopSubPath & strLabelPDSubPath & kLabelSmallSubPath & kLabelSmallLineBase & iElement+1 )
'-		contWork.Active = FALSE
'-	Next

	' add transition animation from pie1 to pie2
'	If iPie = 0 Then
''		CStr(CInt(sGraphicsData.aGroup[iGroup].aAnimOrderFlag[iElement]) + 1)
''		Scene._PlayoutAnimationDirectorNameAdd( "ANI_PIE1TO2", CStr(CInt(sGraphicsData.aGroup[iGroup].aAnimOrderFlag[iElement]) + 1) )
'		' add shift split animation to _PlayoutAnimation
'		Scene._PlayoutAnimationSubDirectorAdd( contBlenderElementIN, "ANI_PIE1TO2" )
'	End If
