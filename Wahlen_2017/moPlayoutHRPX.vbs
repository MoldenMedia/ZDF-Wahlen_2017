...
Dim theDateModified     As String = "27.04.2017 jn"
...
Dim kBGMaxHeightPD As Double = 214.5
...
'println "[sGroupData.dblMinValue] [sGroupData.dblMaxValue]: ["	& sGroupData.dblMinValue & "] [" & sGroupData.dblMaxValue & "]********************" 
	...
	If sGraphicsData.strTypeOfGraphic = "HRPZ" Then
		...
		' jn: fits always with that value...no clue why
		fMaxVizValue = 167
...
'println "DEBUG: [zeroPosY]: ["	& dblZeroPosY & "]" 
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		For iElement = 0 To sGraphicsData.aGroup[iGroup].nElements - 1
			...
			' set scaling of bar label text and visibility of unit percent
			if true then ' scope
				dim dblTextScale As Double
				If sGraphicsData.blnInfoPercentFlag Then
					dblTextScale = 1.0
				else
					dblTextScale = 0.8
				end if		
				...
				showUnit = Not sGraphicsData.blnInfoPercentFlag
				if showUnit and sGraphicsData.aGroup[iGroup].aValueTxt[iElement] == "" then
					showUnit = false
				end if

				' i do it for both, normal and difference, costs nothing

				dim contTmp As Container
				contTmp = contElement.FindSubContainer(kDataSubPath & kTextDataSubPath & kTextValueSubPath & "$JUSTIFY")		
				contTmp.Scaling.x = dblTextScale
				contTmp.Scaling.y = dblTextScale
				contTmp.Scaling.z = dblTextScale
				contTmp.FindSubContainer("$txt_unit").Active = showUnit
			
				contTmp = contElement.FindSubContainer(kDataSubPath & kTextDataSubPath & kTextValueDiffSubPath & "$JUSTIFY")
				contTmp.Scaling.x = dblTextScale
				contTmp.Scaling.y = dblTextScale
				contTmp.Scaling.z = dblTextScale
				contTmp.FindSubContainer("$txt_unit").Active = showUnit
...
'				contElement.FindSubContainer( kDataSubPath & kBarColoredSubPath ).Position.Y = dblZeroPosY
				...
				' set text value and labels
				' difference goes into big label, there is no small label
...
Sub updateScene_LabelOffsetGroup(typeOfGraphic As String, contGroup As Container, nLabels As Double)
	...
	whitec.Position.Y = (nLabels-1)*kBannerStep/2.0

	' set height of group background
	dim bgNogHei as double

	if typeOfGraphic = "HRPZD" then
		' this needs special numbers
		if nLabels = 1 then
			bgNogHei = 208
		elseif nLabels = 2 then
			bgNogHei = 206.5
		...
		If typeOfGraphic = "HRPG" Then
			bgNogHei = kBGMaxHeightPD - (nLabels-1)*kBannerStep
		Else ' "HRPZ" or "HRPD"
			bgNogHei = kBGMaxHeightPZ - (nLabels-1)*kBannerStep
		end if
	End If

	dim bgCont as Container
	bgCont = contGroup.FindSubContainer( kGroupBGSubPath )
	bgCont.Geometry.PluginInstance.SetParameterDouble("height", bgNogHei)
End Sub
'-------------------------------------------------------------------------------
'
Sub updateScene_LabelOffsetElement(typeOfGraphic As String, contElement As Container, nLabels As Double)
	...
	ElseIf typeOfGraphic = "HRPD" Then
		bgMaxHeight = sGlobalParameter.dblMaxVizValueHRPD
	End If

	' set height of animation end keyframe
	dim tmp as double
	tmp = bgMaxHeight - (nLabels-1)*kBannerStep
	contElement.FindSubContainer( kBarBGSubPath ).FindKeyframeOfObject("k_value").FloatValue = tmp	
End Sub
'-------------------------------------------------------------------------------
