...
Sub updateScene_assignData()
	...
	For iGroup = 0 To sGraphicsData.nGroups
		...
		if true then
			...
			' the white rect
			bgc = contGroup.FindSubContainer( kGroupBGSubPath )

			dim bgnog, whitenog as double
			...
			bgc.Geometry.PluginInstance.SetParameterDouble("height", bgnog)
			whitec.FindKeyframeOfObject("k_value").FloatValue = whitenog
		end if
		...
		For iElement = 0 To sGraphicsData.aGroup[iGroup].nElements - 1
			...
			If iElement = 0 Or iElement = 2 Then
				...
				if true then
					Dim transp as Container
					transp = contElement.FindSubContainer( "$transparent" )

					dim transnog as double
					...
					elseif nVisibleLabel = 2 then
						transnog = 93.1
						contElement.Position.Y = 12
					elseif nVisibleLabel = 3 then
						transnog = 86
						contElement.Position.Y = 24
					end if

					transp.FindKeyframeOfObject("k_value").FloatValue = transnog
				end if	
			...
			ElseIf iElement = 1 Then
				...
				if true then
					...
					datac = contElement.FindSubContainer( "$DATA" )
				...
				If CDbl( sGraphicsData.aGroup[0].aValueNum[1] ) = 0.0 Then
					...
					' set arrow label
					' +-
					contElement.FindSubContainer( kDataSubPath & kTextValueSubPath & kTextSubPath ).Geometry.Text = Chr(177) & sGraphicsData.aGroup[0].aValueTxt[1]
