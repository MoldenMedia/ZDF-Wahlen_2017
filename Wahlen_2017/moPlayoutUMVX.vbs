...
' CHANGES
'-------------------------------------------------------------------------------
' BG: use aValueNum instead of aDiffNum for UMVD
'-------------------------------------------------------------------------------
' global definitions
'-------------------------------------------------------------------------------
...
' FIX THIS!!! Temporary overwrite
				...
				dblTempMaxValue = Scene._getAbsMaxBaxValue( sGroupData.aValueNum )
			Else
				dblTempMinValue = 0.0
				dblTempMaxValue = Scene._getMaxBaxValue( sGroupData.aValueNum )
			End If
	...
	ElseIf sGraphicsData.strTypeOfGraphic = "UMVP" Then
		fMinVizValue = 0.0
	...
	If sGraphicsData.strTypeOfGraphic = "UMVPD" Then
		if nMaxLabel = 3 then
			if (fMaxVizValue > 148.0) then
				fMaxVizValue = 148.0
			end if
		elseif nMaxLabel = 2 then
			if (fMaxVizValue > 158.0) then
				fMaxVizValue = 158.0
			end if
		end if
...
Sub updateScene_LabelOffsetGroup(typeOfGraphic As String, contGroup As Container, nLabels As Double)
	...
	whitec.FindKeyframeOfObject("k_value").FloatValue = kBannerHeight + (nLabels-1)*kBannerStep

	Dim bgposc as Container
	bgposc = contGroup.FindSubContainer( "$objGroupBG_pos" )

	If typeOfGraphic = "UMVPD" Or typeOfGraphic = "UMVP" Then
		Dim bgc as Container
		bgc = contGroup.FindSubContainer( "$objGroupBG" )
	
		' set Y position for center of banner
		if nLabels = 1 then
			whitec.Position.Y = 0.0
		elseif nLabels = 2 then
			whitec.Position.Y = 4.0
		elseif nLabels = 3 then
			whitec.Position.Y = 8.0
		end if
		
		' set height of group background
		dim bgheight as double
			
		If typeOfGraphic = "UMVPD" then
			bgc.Position.Y = 13
			bgheight = 212.0
		ElseIf typeOfGraphic = "UMVP" then
			if nLabels = 1 then
				bgc.Position.Y = 12.0
				bgheight = 212.0
			elseif nLabels = 2 then
		...
		bgc.Geometry.PluginInstance.SetParameterDouble("height", bgheight)

		' these are moved only for difference bars
		bgposc.Geometry.PluginInstance.SetParameterDouble("height", 126)

		' groups are not shifted vertically
		contGroup.Position.Y = 0
	elseif typeOfGraphic = "UMVD" then	
		Dim bgnegc as Container
		bgnegc = contGroup.FindSubContainer( "$objGroupBG_neg" )
	
		dim groupbgposheight, groupbgnegheight as double

		' set Y position for center of banner, background of bar
		' shift groups up if there are multiline labels
		if nLabels = 1 then
			whitec.Position.Y = 0
			contGroup.Position.Y = 0
			groupbgposheight = 127
			groupbgnegheight = 85
			bgnegc.Position.Y = -12
		elseif nLabels = 2 then
			whitec.Position.Y = -6
			contGroup.Position.Y = 6
			groupbgposheight = 121
			groupbgnegheight = 79
			bgnegc.Position.Y = -24
		elseif nLabels = 3 then
			whitec.Position.Y = -12
			contGroup.Position.Y = 12
			groupbgposheight = 115.5
			groupbgnegheight = 73
			bgnegc.Position.Y = -36
		end if

		bgposc.Geometry.PluginInstance.SetParameterDouble("height", groupbgposheight)
		bgnegc.Geometry.PluginInstance.SetParameterDouble("height", groupbgnegheight)
	end if
End Sub
'-------------------------------------------------------------------------------
'
...
Sub updateScene_LabelOffsetElement(typeOfGraphic As String, contElement As Container, nLabels As Double)
	If typeOfGraphic = "UMVPD" Or typeOfGraphic = "UMVP" Then
		...
		Dim ctransp, cobjgeom, clab1, clab2, clab3 as Container
		Dim shifty As Double

		ctransp = contElement.FindSubContainer( "$GFX_ELE$transparent" )
		cobjgeom = contElement.FindSubContainer( "$obj_geom" )
		clab1 = contElement.FindSubContainer( "$TXT_LABEL_1" )
		clab2 = contElement.FindSubContainer( "$TXT_LABEL_2" ) 
		clab3 = contElement.FindSubContainer( "$TXT_LABEL_3" )

		If typeOfGraphic = "UMVPD" Then
			Dim clabeltextgdiff, clabelrectgdiff as Container
			clabeltextgdiff = contElement.FindSubContainer( "$TXT_VALUE_DIFF$JUSTIFY" )
			clabelrectgdiff = contElement.FindSubContainer( "$TXT_VALUE_DIFF$bg_top" )
			...
			clabelrectgdiff.Position.Y = -21.15 + shifty
			clabeltextgdiff.Position.Y = 0.0 + shifty

			ctransp.Position.Y = 0.0 + shifty
			cobjgeom.Position.Y = 0.0 + shifty
			clab1.Position.Y = -60.0 + shifty
			clab2.Position.Y = -60.0 + shifty
			clab3.Position.Y = -60.0 + shifty
			contElement.Position.Y = 0.0

			ctransp.FindKeyframeOfObject("k_value").FloatValue = 100
		Else
			dim traspnog as double
			...
			ctransp.Position.Y = 0.0
			cobjgeom.Position.Y = 0.0
			clab1.Position.Y = -60.0
			clab2.Position.Y = -60.0
			clab3.Position.Y = -60.0
			contElement.Position.Y = 0.0 + shifty

			ctransp.FindKeyframeOfObject("k_value").FloatValue = traspnog
		End If

	elseif typeOfGraphic = "UMVD" then
		Dim ctranspneg, ctransppos, credneg as Container
		...
		credneg = contElement.FindSubContainer( "$DATA$obj_geom_neg" )
		
		dim transpnoggi As Double

		if nLabels = 1 then
			credneg.Position.Y = -74.0
			ctranspneg.Position.Y = -74.0
			transpnoggi = 100
		elseif nLabels = 2 then
			credneg.Position.Y = -85.6
			ctranspneg.Position.Y = -85.6
			transpnoggi = 92.5
		elseif nLabels = 3 then
			credneg.Position.Y = -98.0
			ctranspneg.Position.Y = -98.0
			transpnoggi = 85
		end if
