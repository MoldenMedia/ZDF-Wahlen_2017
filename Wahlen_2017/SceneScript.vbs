...
'	RegisterParameterDouble("theMaxVizValueHROWP", "max Viz value HROWP", 100.0, 50.0, 200.0)
'	RegisterParameterDouble("theMaxVizValueHROWPD", "max Viz value HROWPD", 100.0, 50.0, 200.0)
...
'	RegisterParameterDouble("theMaxVizValueUMHP", "max Viz value UMHP", 190.4, 50.0, 200.0)
...
'	RegisterParameterDouble("theMaxVizValueUMKB", "max Viz value UMKB", 190.0, 10.0, 200.0)
...
'	sGlobalParameter.dblMaxVizValueHROWP       = GetParameterDouble("theMaxVizValueHROWP")
'	sGlobalParameter.dblMaxVizValueHROWPD      = GetParameterDouble("theMaxVizValueHROWPD")
...
'	sGlobalParameter.dblMaxVizValueGraphHeight = GetParameterDouble("theMaxVizValueGraphHeight")
	...
	sGlobalParameter.dblMaxVizValueHRWB        = 167.0
	...
	sGlobalParameter.dblMaxVizValueUMHPD       = 150.0
	sGlobalParameter.dblMaxVizValueUMHP_noGrp  = 285.7
	sGlobalParameter.dblMaxVizValueUMHPD_noGrp = 253.0
	...
	sGlobalParameter.dblMaxVizValueANVP        = 167.5
	sGlobalParameter.dblMaxVizValueANVPD       = 167.5
	...
	sGlobalParameter.dblANLabHeight            = 3.8
	sGlobalParameter.dblMaxVizValuePBPJ        = 180.0
	sGlobalParameter.dblMinVizValuePBRK        = 0.138
	sGlobalParameter.dblMaxVizValuePBRK        = 0.768
	sGlobalParameter.dblMaxVizValueGraphWidth  = 105.0
	sGlobalParameter.dblMaxVizValueGraphHeight = 223.0
...
Sub _updateScene_assignLabel_3_2017(contLabelBase As Container, strTypeOfGraphics As String, strValue As String, strLabel1 As String, strLabel2 As String, strLabel3 As String, dblValue As Double, nVisibleLabel As Integer)
	...
	' NOTE: this is using some logic to hide the unit (%)
	' but some scripts have its own logic, contradicting to the scene script - the bool flag show info percent
	' now what...

	if strTypeOfGraphics.Match( "[HRPZ|HRPD|HRPG|HRPZD|UMVB|UMVB|HRWB|ANSVZP|ANSVZD]" ) Then
		' do not touch the unit! script handles it
		' this is how it should be. others are left here as they were for compatibility reasons
	ElseIf strValue = "" And strTypeOfGraphics.Match( "[HRWB|UMVP|HROWVP|ANNQRP|ANSVZP|PBPJ|UMWW]" ) Then
		contWork1.FindSubContainer( kUnitSubPath ).Active = FALSE
	ElseIf strTypeOfGraphics.Match( "UMVD|HROWVD|ANNQRD|ANSVZD|PBPJPD" ) Then
		contWork1.FindSubContainer( kUnitSubPath ).Active = FALSE
	Else
...
'-------------------------------------------------------------------------------
'
Sub _updateScene_assignDiffLabel_3_2017(contLabelBase As Container, strTypeOfGraphics As String, strValueP As String, strValueD As String, strLabel1 As String, strLabel2 As String, strLabel3 As String, nVisibleLabel As Integer)
	Dim strDebugLocation As String = strScriptName & "_updateScene_assignDiffLabel_3():"
	Dim contWork1, contWork2, contWork3 As Container
	Dim strTemp As String
	...
	if strTypeOfGraphics = "UMVBPD" Then
		' do not touch the unit or visibilities! script handles it
		contWork1 = contLabelBase.FindSubContainer( kTextValueDiffSubPath )
	ElseIf strTypeOfGraphics = "ANSVZPD" Then
		' do not touch the unit or visibilities! script handles it
		contWork1 = contLabelBase.FindSubContainer( kTextValueSubPath )
	ElseIf strTypeOfGraphics.Match( "[UMVPD|UMAS|HROWVPD|UMWW]" ) Then
		' left here as they were for compatibility reasons
		contLabelBase.FindSubContainer( kTextValueSubPath ).Active = FALSE
		contWork1 = contLabelBase.FindSubContainer( kTextValueDiffSubPath )
		contWork1.Active = TRUE
...
'println "_updateScene_assignPieLabel: [contLabelBase.FindSubContainer( kPieTextValueSubPath ).Active]: [" & contLabelBase.FindSubContainer( kPieTextValueSubPath ).Active & "]______________________________"
	...
	' set label text
' ##############################################################################
'  hier viertes Label fuer Bremen ###############################################
' ##############################################################################
...
Function _getMaxBaxValue( aValueNum As Array[String] ) As Double
	' beware. this is not true maximum. it does Max(0, Max(array))
	' it cannot find maximum of array elements if all are <0. it returns 0 instead
	Dim iCnt As Integer
	Dim dblValue, dblMaxValue As Double
...
Function _getMinBaxValue( aValueNum As Array[String] ) As Double
	' beware. this is not true minimum. it does Min(0, Min(array))
	' it cannot find minimum of array elements if all are >0. it returns 0 instead
	Dim iCnt As Integer
	Dim dblValue, dblMinValue As Double
...
'-------------------------------------------------------------------------------
'
Sub _POA_PlayShowF0ALL( strType As String, strEventList As String )
	Dim iCnt As Integer
	Dim aPOEventList  As Array[structPlayoutAnimEvent]
	...
	For iCnt = 0 To aPOEventList.UBound
		If strType = aPOEventList[iCnt].strType Then
			System.SendCommand( "#" & aPOEventList[iCnt].dirAnimation.vizID & " SHOW F0" )
		End If
	Next
End Sub
'-------------------------------------------------------------------------------
...
'-------------------------------------------------------------------------------
'
Function _getMaxStrArrayValue( aValueNum As Array[String], deflt as Double ) As Double
	Dim iCnt As Integer
	Dim t, ret As Double
	
	ret = deflt
	For iCnt = 0 To aValueNum.UBound
		t = CDbl( aValueNum[iCnt] )
		If t > ret Then
			ret = t
		End If
	Next
	_getMaxStrArrayValue = ret
End Function
'-------------------------------------------------------------------------------
'
Function _getMinStrArrayValue( aValueNum As Array[String], deflt as Double ) As Double
	Dim iCnt As Integer
	Dim t, ret As Double
